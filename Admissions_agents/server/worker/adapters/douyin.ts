...
const INBOX_URL = 'https://creator.douyin.com/creator-micro/home';
const LOGIN_CHECK_URL = 'https://creator.douyin.com';
...
const parseCookies = (cookiesJson: string | null): unknown[] | null => {
  ...
  try {
    ...
    logger.warn('douyin', '解密 cookies 失败', { error: message });
    return null;
  }
};
...
export async function publishToDouyin(
  ...
  try {
    ...
    const loggedIn = await page.locator('text=/发布视频|发布作品|上传/').first().isVisible({ timeout: 5000 }).catch(() => false);
    if (!loggedIn) {
      return { success: false, message: '登录态已失效，请重新登录账号' };
    }

    await page.locator('text=图文').first().click({ timeout: 5000 }).catch(() => null);
    await humanDelay();
    ...
    if (process.env.RPA_AUTO_SUBMIT === 'true') {
      ...
      return { success: true, message: '抖音图文已提交' };
    }

    return {
      success: false,
      message: 'RPA_AUTO_SUBMIT 未启用，抖音内容已填入但未点击发布',
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    logger.error('douyin', '发布异常', { accountId: account.id, error: message });
    return { success: false, message: `发布异常：${message}` };
  }
}

export async function fetchDmFromDouyin(
  account: AccountContext,
  ...
  try {
    ...
    await page.locator('text=/消息|私信/').first().click({ timeout: 5000 }).catch(() => null);
    await humanDelay(2000);

    const rawItems = await page.evaluate(() => {
      const nodes = document.querySelectorAll('[data-e2e*="message"], .message-item, .msg-item');
      return Array.from(nodes)
        .slice(0, 50)
        .map((node, idx) => {
          const el = node as HTMLElement;
          return {
            id: el.getAttribute('data-id') || `dy-${Date.now()}-${idx}`,
            sender: (el.querySelector('.nickname, .user-name, .name') as HTMLElement | null)?.innerText?.trim() || '未知用户',
            content: (el.querySelector('.content, .last-msg, .text') as HTMLElement | null)?.innerText?.trim() || '',
          };
        });
    });
    ...
    const message = error instanceof Error ? error.message : String(error);
    logger.error('douyin', '抓取异常', { accountId: account.id, error: message });
    return {
...
export async function captureDouyinLoginCookies(
  account: AccountContext
  ...
  try {
    ...
    logger.info('douyin', '请扫码登录抖音创作者中心，最多等待 3 分钟', { accountId: account.id });

    const loggedIn = await page
      .locator('text=/创作中心|首页|发布作品|数据/')
      .first()
