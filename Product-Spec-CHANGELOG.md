...
> ① 学员声音从来没回到平台（乙方做不好平台不知道 / 监管来查没证据 / 续约决策只有甲方一面之词）
> ② 线索 Detail 是流水账，看不出"卡在哪一步"
> ③ 三个 AI 员工是同事但只有「小报」开口，小招/小线像背景人
> **本迭代主题**：让平台**听到学员**、让专员**看清旅程**、让 AI 同事**各说各的**。

### v3.6.a · 学员评价系统（监管武器 + 续费证据）

- **数据库**（`server/src/db.ts`）：新建 `student_evaluations` 表
  - 5 维度评分（顾问态度 / 学习指导 / 缴费透明 / 材料办理 / 总体）
  - `avg_score` 自动算 · 任一维度 ≤ 2 自动标 `is_complaint = 1`
  - 一个 lead 30 天内只能评 1 次（防刷）
- **路由**（`server/src/routes/evaluations.ts`）：
  - `GET /api/student/evaluations/eligible` 学员侧检查是否可评
  - `POST /api/student/evaluations` 提交（仅 enrolled/studying/graduated 状态可评）
  - `GET /api/evaluations` 后台聚合（统计 + 列表 + 投诉率）
  - `GET /api/evaluations/by-tenant` 平台跨租户对比 + at_risk 标记（投诉率 ≥ 30% 或均分 < 3）
- **学员端 Portal**（`StudentPortalEntry.tsx`）：新增 `EvaluationWidget`
  - 检测到可评状态时显示橙色「⭐ 给我们打个分」按钮
  - 5 个 1-5 分滑块 + 500 字反馈
  - 已评则显示"30 天后可再评一次"
- **后台**：复用 `ComplianceCenter` 加「学员评价」tab（投诉本质上是合规信号）
  - 4 个统计卡片（总数/均分/投诉数/投诉率）
  - 「全部 / 只看投诉」筛选
  - 投诉行红底高亮

### v3.6.b · 学员旅程时间线

- **后端**：`GET /api/leads/:id/journey` 聚合 8 个生命周期事件
  - 测评/留资 → 首次跟进 → 高意向识别 → 缴定金 → 签约成交 → 完成报名 → 学员评价
  - 算 `stagnantDays` + `isStuck`（停滞 ≥ 7 天且未到结束态）
- **前端**：`LeadManagement.tsx` 详情面板加蓝色「🗺️ 学员旅程」卡片
  - 8 类阶段图标（📝/👋/🔥/💰/✅/🎓/⭐）
  - "卡住 N 天" amber 警示
  - 每个事件含时间 + detail（如"¥2000 定金"/"小红书来源"）
  - 提示下一步动作（基于 currentStage 给一句"建议把这条线索推到下一步 X"）

### v3.6.c · AI 员工自己的声音

- **`briefing-generator.ts` 重写**：从「小报代笔总结」→「三段式三人小队日报」
  - 🎯 小招说：内容相关数据 + 1 句明天打算
  - 🕸️ 小线说：私信/线索数据 + 1 句明天打算
  - 📊 小报说：综合点评 + 给老板 1 个建议
- Gemini prompt 引导按三段式输出 · 模板回退也同样三段式
- 每段都有"明天我打算"或"建议"句，体现"会思考"
- 失败数 > 0 必须由小报段提到 · 数据全 0 时诚实表达

### 度量目标

- **v3.6.a**：60 天后已 enrolled 学员评价率 ≥ 30% · 投诉率高于 20% 的乙方触发平台主动接触
- **v3.6.b**：专员看完旅程后将卡住 ≥ 7 天的线索推进比例 ≥ 25%
- **v3.6.c**：每日战报阅读率（企微推送回执）≥ 70%（之前因为只有"小报代笔"略平淡）
